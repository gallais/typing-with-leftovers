     ...
     { % premise 3
      \inferrule
       { % premise 4
        ...
        [] ∙ \fresh{σ ⊗ τ} ⊢_v \texttt{0} ∈ σ ⊗ τ \andalso{} [] ∙ \stale{σ ⊗ τ}
      }
    }{ % conclusion 3
       [] ∙ \fresh{σ ⊗ τ} ⊢ \texttt{var}(\texttt{0}) ∈ σ ⊗ τ \andalso{} [] ∙ \stale{σ ⊗ τ}
     }
   \and % premise 2
    \inferrule
     { % premise 3
...
Π = \inferrule
        ...
        { % premise 4
         \inferrule
          { % premise 5
           ...
           [] ∙ \stale{σ ⊗ τ} ∙ \fresh{τ} ⊢_v \texttt{0} ∈ τ \andalso{} [] ∙ \stale{σ ⊗ τ} ∙ \stale{τ}
         }
       }{ % conclusion 4
         [] ∙ \stale{σ ⊗ τ} ∙ \fresh{τ} ∙ \fresh{σ} ⊢_v \texttt{1} ∈ τ
          \andalso{} [] ∙ \stale{σ ⊗ τ} ∙ \stale{τ} ∙ \fresh{σ}
       }
     }{ % conclusion 3
  ...
  \and Π'
   ...
   \texttt{prd}(\texttt{neu}(\texttt{var}(\texttt{1})),~\texttt{neu}(\texttt{var}(\texttt{0}))
   \andalso{} [] ∙ \stale{σ ⊗ τ} ∙ \stale{τ} ∙ \stale{σ}
 }
