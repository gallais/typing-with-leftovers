...
app-inv :
  ...
  Γ ⊢ `app t u ∈ τ ⊠ Δ → Σ[ θ ∈ Usages γ ] Σ[ σ ∈ Type! ] Γ ⊢ t ∈ σ ⊸ τ ⊠ θ × θ ⊢ σ ∋ u ⊠ Δ
app-inv (`app t u) = , , t , u
...
case-inv : 
  ...
  Γ ⊢ t ∈ σ ⊕ τ ⊠ θ × fresh σ ∷ θ ⊢ ν₁ ∋ l ⊠ stale σ ∷ Δ × fresh τ ∷ θ ⊢ ν₁ ∋ r ⊠ stale τ ∷ Δ
...
lam-inv : 
  ...
  Γ ⊢ σ ⊸ τ ∋ `lam t ⊠ Δ → fresh σ ∷ Γ ⊢ τ ∋ t ⊠ stale σ ∷ Δ
lam-inv (`lam t) = t
...
app-inv-function : 
  {n : ℕ} {γ : Context n} {t : Infer n} {u : Check n} {Γ Δ : Usages γ} {τ : Type!} →
  (p : Γ ⊢ `app t u ∈ τ ⊠ Δ) → let (θ , σ , _) = app-inv p in Γ ⊢ t ∈ σ ⊸ τ ⊠ θ
...
case-inv-left : 
  {n : ℕ} {γ : Context n} {t : Infer n} {l r : Check (suc n)} {Γ Δ : Usages γ} {ν₁ ν₂ : Type!} →
  (p : Γ ⊢ `case t return ν₁ of l %% r ∈ ν₂ ⊠ Δ) →
  let (θ , σ , τ , _) = case-inv p in fresh σ ∷ θ ⊢ ν₁ ∋ l ⊠ stale σ ∷ Δ
case-inv-left p = let (_ , _ , _ , _ , L , _) = case-inv p in L

case-inv-right : 
  {n : ℕ} {γ : Context n} {t : Infer n} {l r : Check (suc n)} {Γ Δ : Usages γ} {ν₁ ν₂ : Type!} →
  (p : Γ ⊢ `case t return ν₁ of l %% r ∈ ν₂ ⊠ Δ) →
  let (θ , σ , τ , _) = case-inv p in fresh τ ∷ θ ⊢ ν₁ ∋ r ⊠ stale τ ∷ Δ
...
let-inv-body : 
  {n o : ℕ} {γ : Context n} {p : Pattern o} {t : Infer n} {u : Check (o + n)}
  {Γ Δ : Usages γ} {τ : Type!} (p : Γ ⊢ τ ∋ `let p ∷= t `in u ⊠ Δ) →
  let (θ , σ , δ , _) = let-inv p in f⌜ δ ⌝ ++ θ ⊢ τ ∋ u ⊠ s⌜ δ ⌝ ++ Δ
